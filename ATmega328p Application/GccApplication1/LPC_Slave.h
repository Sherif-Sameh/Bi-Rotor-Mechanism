 ...
 *       * MSG ID   |   MessageType     |  Length of message in Words (1 word = 4 bytes)
         ...
         *    7     |   Reserved        |                            -
 * 
 * @param start_byte This is the first byte recieved during an LPC interaction
 ...
 * 
 * @param msg_recieved the full message as recieved from master
 * @param msg_length Length of the msg_recieved array
 * @return true 
 * @return false 
 */
bool compare_crc8(uint8_t* msg_recieved, uint8_t msg_length);

/**
 * @brief Decodes the LPC message
 * 
 * @param msg_recieved The entire recieved message, including the crc
 * and start bytes
 * @param msg_id The id of the recieved message.
 ...
 */
msg_type_t LPC_decode(uint8_t* msg_recieved,uint8_t msg_id);
...
/**
 * @brief Get the decoded data that we recieved and copies it to an array
 * input by the user
 * 
...
/**
 * @brief a method to recieve data via LPC
 * 
 * @return msg_type_t 
 */
