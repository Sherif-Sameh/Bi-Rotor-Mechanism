...
bool compare_crc8(uint8_t* msg_recieved,uint8_t msg_length){
    uint8_t crc = CRC8Calculate((msg_recieved+1),msg_length);
    if (crc == *(msg_recieved+msg_length+1))
        return true;
    return false;
}
    ...
    switch(msg_id){
        ...
        case 1:
            for(uint8_t i=0;i<8;i++)
                convert_to_float.byte_num[i] = *(msg_recieved +1+i);
            free(msg_recieved);
            //delete msg_recieved;
        ...
        case 4: free(msg_recieved); //delete msg_recieved; 
        return MSG_TYPE_ACK;
        case 5: free(msg_recieved); //delete msg_recieved; 
        return MSG_TYPE_NACK;
        default: free(msg_recieved); //delete msg_recieved; 
        return MSG_FAILURE;
    }
}
